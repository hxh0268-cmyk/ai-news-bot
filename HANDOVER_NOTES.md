...
## 次にやること
- 【最重要・2週間の実験開始】広報・配信を本気で開始する（9/24決定）。
  背景：サイトのエンジニアリング面（改善案10項目＋次フェーズ3項目）は作り込んだ一方、
  広報・配信には一切着手していなかった。9/24にBuffer Insightsを確認したところ、
  総フォロワー数1人、エンゲージメントほぼ0という状態だったことから、「AIニュースという
  テーマ自体を変えるべきか」を検討した。結論として、テーマの問題と「配信していない」
  問題を混同すべきではないと判断し、以下の方針とした：
  - テーマ（AIニュース）は変更しない。テーマを変えても、配信をしなければ同じ結果になる
  - 2週間、以下を本気で行う：①自動化・Buffer経由の投稿とは別に、個人アカウントから
    週3〜4回、生身の言葉で手動紹介する、②AI関連のコミュニティ（Slack/Discord/X等）に
    最低1つ、継続的に顔を出す
  - 【9/24追記・重要な軌道修正】Xで使う「Bal082229」アカウントは、普段使いの
    メインアカウントではなく、本プロジェクトのために新規開設したフォロワー0・
    投稿履歴0のアカウントだと判明。新規アカウントの単独投稿はXのアルゴリズム上
    ほぼ拡散されないため、当初の「個人アカウントから単独投稿する」戦術だけでは
    10/8までに反応を得られる可能性が低いと判断し、以下に戦術を修正した：
    - プロフィール（アイコン・自己紹介文）を先に整える
    - 単独投稿は週1〜2回に減らし、アカウントの投稿履歴を積む目的に留める
    - 週3〜4回の主軸は、AI関連の話題で既にフォロワーがいるアカウントの投稿に、
      自分の意見でリプライすること（フォロワー0でも拡散に頼らず見られる数少ない方法）
  - 【9/25追記・進捗と評価基準】プロフィール整備（アイコン・表示名・自己紹介文・
    リンク欄）を完了し、リプライ活動を開始（初日3件）。実施の中で以下の評価基準を
    確立した。今後リプライ内容をレビューする際はこの基準を使う：
    1. 元投稿の具体的な事実・数字に触れているか（テンプレート的な相槌にならない）
    2. 自分の意見を言い切っているか（「〜なのか？」等の問いかけで終わらない）
    3. 政治色の強い投稿は避ける（テーマの検証という目的から外れ、不要な炎上
       リスクを負うだけのため）
    4. そのスレッド内で言及されていない事実を、既知の事実であるかのように
       書かない（他の記事で見た情報を根拠なく持ち込まない。初日、価格「不明」と
       明記された投稿に「価格も手を出しやすい」と矛盾する発言をした事例あり、
       事後修正済み）
    ペースの目安：1回の活動で2〜3件、週3〜4回（2週間で12〜24件程度）
  - 判断基準（10/8目安）：見知らぬ誰かからの自発的な反応（いいね以上、コメント・
    シェア・DM等）が1件でもあったかどうか。満たせば継続に自信を持ってよい。
    満たさなければ、その時初めてテーマ変更・アプローチの根本見直しを検討する
  - 教訓：机上の調査（競合分析・市場調査等）だけでは意思決定できない段階に来ている。
    今回は「調べる」ではなく「本人が実際に2週間、配信行動をやってみる」ことを優先した
- ナレーション機能（ElevenLabs）継続判断は、広報活動を始めるまで保留（9/24更新）。
  経緯：9/23、Cursorから「ナレーションを除外してはどうか」という提案があり、
  「Bufferの投稿データ（ナレーション付き/無音フォールバック）を比較して10/7目安で
  再判断する」という計画を立てた。しかし9/24、実際にBuffer Insightsを確認したところ、
  総フォロワー数1人、個別投稿のViews/Reactions/Comments/Shares/Saves全て0という状況
  だった。これは異常ではなく、**現在はプロジェクトの基盤作り段階で広報活動を
  一切行っていないため**という理由が確認できた。つまりエンゲージメントを判断材料に
  する計画は、広報活動を始めるまでそもそも実行不可能であり、10/7という期限にも
  根拠がなかったため撤回する。
  結論：ナレーションは完成済み・正常動作している機能であり、月$6というコストは
  基盤作り段階の出費としては誤差レベルのため、わざわざ外す理由がない。上記の
  広報実験（10/8目安）とあわせて、リーチ・エンゲージメントのデータが実際に
  意味を持つようになった段階で、改めて判断する
