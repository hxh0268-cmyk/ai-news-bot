...
【アートディレクション：静謐なSFミニマリズム】
目指すのは「壮大でがらんとした空間」と「ささやかな人間の営み」を対比させる構図です。
派手なガジェット描写や説明的なテクノロジーの絵は避け、その先にある人間の情緒を描いてください。

- 構図：画面の6〜7割を、宇宙的または建築的スケールの「何もない空間」にする（例：軌道上のステーション内部から見える惑星の弧、がらんとした巨大な建築空間、夜明け前の静かな都市）。
  その中に、ごく小さく・私的な人間の営みを1つだけ配置する（例：ひとりで何かを書き記す人影、小さな灯りのそばで手を動かす人、静かに何かを見つめる後ろ姿）
- 対比：先端的な空間の中に、祖先から続くような素朴な仕草や道具を同居させる（技術そのものは描かず、気配としてだけ存在させる）
- トーン：ディストピアでもユートピアでもない、静かで両義的な余韻
- 光：単一方向からの柔らかい光（遠い惑星の光、夜明け前の薄明かりなど）。強いコントラストやネオン的な光は避ける
- 配色：彩度を落とした、映画のカラーグレーディングのような落ち着いたトーン
- 人物を描く場合は、顔の判別できない小さなシルエット・後ろ姿程度に留める
- 文字・ロゴ・ウォーターマーク・UIパーツ・具体的なガジェット（スマホ画面等）は一切入れない
- 縦長構図。画面の下半分は特に「何もない余白」を多く残す（後で見出しテキストを重ねるため）
`.trim();
}
