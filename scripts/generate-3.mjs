...
【JSON形式を厳守するための注意】
- 本文・見出し・キャプションなどの文章中では、半角のダブルクォート（"）を絶対に使わないでください。何かを引用したい場合は、必ず日本語の鉤括弧「」を使ってください
- 文章中で改行はしないでください（1つの値は1行の文字列にしてください）
- 出力の最後は、必ず配列を閉じる「]」で終わらせてください。書きかけの状態で終わらせないでください
`.trim();
...
async function callClaude() {
  ...
  try {
    ...
    const contextStart = Math.max(0, errorPos - 300);
    const contextEnd = Math.min(jsonStr.length, errorPos + 300);
    console.error("JSON解析エラー:", err.message);
    console.error("問題箇所の前後600文字:\n" + jsonStr.slice(contextStart, contextEnd));
    throw err;
  }
}
